...
def add(num):
    ...
    heapq.heappush(small, -1 * num) # Since max heap  small = [-7, -1]

    # We need to check first if small has values less than large to move elements; then check uneven length or size to move elements
    # Check if values in the small heap is lesser than the values in the large heap or not; if not we move the elements
    if small and large and (-1 * small[0]) > large[0]: #  7 > 2 yes
        val = - 1 * heapq.heappop(small) # small = [-1]
